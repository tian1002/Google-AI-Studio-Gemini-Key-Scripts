...
do
  ...
  if [ "$ACTION_MODE" == "CREATE_AND_EXPORT_NEW" ]; then
      ...
      if [ $LIST_EXIT_CODE -ne 0 ]; then
          ...
          elif echo "$LIST_KEYS_STDERR" | grep -q "consumer does not have access"; then
      ...
      # --- 修改后的 JQ 处理逻辑 ---
      # 首先检查 ALL_KEYS_JSON 是否确实是一个数组
      if echo "$ALL_KEYS_JSON" | jq -e 'type == "array"' > /dev/null; then
          # 如果它是一个数组，直接迭代数组中的每个元素 (每个元素都是一个密钥对象)
          echo "$ALL_KEYS_JSON" | jq -c '.[]?' | while IFS= read -r key_json; do
              ...
              KEY_DISPLAY_NAME=$(echo "$key_json" | jq -r '.displayName // "(无显示名称)"')
              KEY_UID=$(echo "$key_json" | jq -r '.uid // "(未知UID)"')
